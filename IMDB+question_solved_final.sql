...
SELECT genre, COUNT(movie_id) AS genre_total 
FROM genre
GROUP BY genre;
...
SELECT genre, COUNT(movie_id) AS number_of_movies, year
FROM genre AS g
JOIN movie AS m
ON g.movie_id = m.id
WHERE year = 2019
...
JOIN ratings AS r
ON r.movie_id = m.id
WHERE MONTH(date_published) = 3 AND year = 2017 AND m.country='USA' AND r.total_votes>1000 IS NOT NULL
GROUP BY g.genre 
ORDER BY movie_count DESC;
...
JOIN ratings AS r 
ON m.id=r.movie_id
WHERE median_rating>=8 AND production_company IS NOT NULL AND POSITION(',' IN languages)
GROUP BY production_company
LIMIT 2;
